...
describe('Module', function () {
  ...
  describe('Custom Code Transform', function () {
    ...
    pit('does not store anything but dependencies if the `cacheTransformResults` option is disabled', function () {
      var mockedResult = {
        code: exampleCode,
        arbitrary: 'arbitrary',
      ...
      transformCode.mockReturnValue(Promise.resolve(mockedResult));
      var module = createModule({ transformCode: transformCode, options: {
    ...
    pit('stores all things if options is undefined', function () {
      var mockedResult = {
        code: exampleCode,
        arbitrary: 'arbitrary',
        dependencies: ['foo', 'bar'],
      ...
      transformCode.mockReturnValue(Promise.resolve(mockedResult));
      var module = createModule({ transformCode: transformCode, options: undefined });

      return module.read().then(function (result) {
        expect(result).toEqual(_extends({}, mockedResult, { source: 'arbitrary(code);' }));
      });
    });
