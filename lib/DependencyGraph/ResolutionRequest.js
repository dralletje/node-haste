  ...
  _createClass(ResolutionRequest, [{
    ...
    value: function _loadAsFile(potentialModulePath, fromModule, toModule) {
      ...
      return Promise.resolve().then(function () {
        if (_this6._helpers.isGlob(toModule)) {
          var basedir = path.dirname(fromModule.path);
          var modulepath = toModule.slice('glob!'.length);
          var filename = path.join(basedir, modulepath);
          return _this6._moduleCache.getGlobModule(filename);
        } else {
